...
def main(args):
    ...
    if args.plotxy:
        for v in range(nvar):
            # plt.figure(v)
            figsize=set_size(74.7, fraction=1.2)
        ...
        fig.savefig("../output/img_xy-real-moving.svg", format='svg', bbox_inches='tight')
        svg_to_emf("../output/img_xy-real-moving.svg")
        plt.show()
        ...
        for i in range(len(datas[0])):
            ...
            for v in range(nvar):
                if i < len(datas[v]):
                    ...
                    ax.plot(index[v]+1, datas[v][0], label="{}[{}]".format(args.variable[v], 0), linewidth=args.linewidth)
            plt.legend(loc='upper center')
        plt.show()
